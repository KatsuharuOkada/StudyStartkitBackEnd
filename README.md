...
```bash
# Create file .env before start docker.
...
- Create migration file: npm run migration:create --name=nameOfMigrationFile
- Generate migration file from entities: npm run migration:generate --name=nameOfMigrationFile
- Run migration: yarn migrations:run

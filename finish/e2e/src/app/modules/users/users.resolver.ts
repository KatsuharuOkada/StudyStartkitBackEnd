...
import { currentUser } from '../../../vendors/decorators/user.decorator';
import { JwtAuthGuard } from '../../../vendors/guards/auth.guard';
import { UserQueryResponse } from '../../graphql/graphql.schema';
import { HttpException, HttpStatus, UseGuards, NotFoundException } from '@nestjs/common';
...
export class UsersResolver extends BaseResolver {
  ...
  @UseGuards(JwtAuthGuard)
  @Query('profile')
  async profile(@currentUser() auth: any): Promise<UserQueryResponse> {
    const user = await this.usersService.getUserInfoByUserId(auth.userId);
    console.log(user);
    if (!user) {
      throw new NotFoundException('Error', 'Data not found.');
    }

...
module.exports = async function generateQueryList(answers) {
  // Root path
  ...
  if (graphqlDataStr.indexOf(answers.apiName) > -1) {
    ...
    let replacedGraphqlData = GenerateUtil.replaceContentForGraphqlQueryList(graphqlDataStr, answers);
  ...
  if (serviceDataStr.indexOf(answers.apiName) > -1) {
    ...
    // Get query list contents
    const querySvListData = FileUtil.readFile('./tools/templates/services/query-list.template');
    const replacedSvQueryListData = GenerateUtil.replaceContentForServiceQueryList(querySvListData.toString(), answers);

    replacedServiceData = replacedServiceData.replace(
      '// Append code in this place',
      '// Append code in this place\n' + replacedSvQueryListData + '\n'
    );
  ...
  if (resolverDataStr.indexOf(answers.apiName) > -1) {
    ...
    // Get query list contents
    const rsQueryListData = FileUtil.readFile('./tools/templates/resolvers/query-list.template');
    const replacedRsQueryListData = GenerateUtil.replaceContentForResolverQueryList(
      rsQueryListData.toString(),
      answers
    );

    replacedResolverData = replacedResolverData.replace(
      '// Append code in this place',
      '// Append code in this place\n' + replacedRsQueryListData + '\n'
    );
    // Write resolver file

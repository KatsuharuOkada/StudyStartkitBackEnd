...
module.exports = async function generateE2EUpdate(answers) {
  ...
  if (FileUtil.existPath(e2eFile)) {
    ...
    let replacedData = GenerateE2EUtil.replaceContentForUpdate(updateData.toString(), answers);
    ...
    if (answers.validateStringInfo) {
      ...
      replacedData = GenerateE2EUtil.replaceAppendContent(
        replacedData,
        GenerateE2EUtil.replaceContentForValidation(validationStrData.toString(), answers.validateStringInfo)
      );
    }
    ...
    if (answers.validateIntInfo) {
      ...
      replacedData = GenerateE2EUtil.replaceAppendContent(
        replacedData,
        GenerateE2EUtil.replaceContentForValidation(validationIntData.toString(), answers.validateIntInfo)
      );
    }

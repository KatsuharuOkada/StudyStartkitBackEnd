...
module.exports = async function generateDelete(answers) {
  // Root path
  ...
  if (graphqlDataStr.indexOf(answers.apiName) > -1) {
    ...
    let replacedGraphqlData = GenerateUtil.replaceContentForGraphqlDelete(graphqlDataStr, answers);
  ...
  if (resolverDataStr.indexOf(answers.apiName) > -1) {
    ...
    // Get delete contents
    const rsDeleteData = FileUtil.readFile('./tools/templates/resolvers/delete.template');
    const replacedRsDeleteData = GenerateUtil.replaceContentForResolverDelete(rsDeleteData.toString(), answers);

    replacedResolverData = replacedResolverData.replace(
      '// Append code in this place',
      '// Append code in this place\n' + replacedRsDeleteData + '\n'
    );
    // Write resolver file
  ...
  if (serviceDataStr.indexOf(answers.apiName) > -1) {
    ...
    // Get delete contents
    const deleteSvData = FileUtil.readFile('./tools/templates/services/delete.template');
    const replacedSvDeleteData = GenerateUtil.replaceContentForServiceDelete(deleteSvData.toString(), answers);

    replacedServiceData = replacedServiceData.replace(
      '// Append code in this place',
      '// Append code in this place\n' + replacedSvDeleteData + '\n'
    );

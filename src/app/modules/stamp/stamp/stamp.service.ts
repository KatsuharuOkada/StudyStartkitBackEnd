...
export class StampsService {
  ...
  async addStamp(userId: string): Promise<MutationResponse> {
    ...
    const userStamp = await this.stampRepository.getStampAddedToday(userId);
    if (userStamp) {
      if (userStamp.reason === AddStampReason.INSTALL) {
        userStamp.count++;
      ...
      const isNotFirstScan = await this.stampRepository.checkFirstScan(userId);
      const newStamp = new UserStampEntity();
      newStamp.count = 1;
      newStamp.reason = isNotFirstScan ? AddStampReason.SCAN : AddStampReason.INSTALL;

...
export class StampRepository extends BaseRepository<UserStampEntity> {
  ...
  async getStampAddedToday(userId: string): Promise<UserStampEntity> {
    try {
      return await this.createQueryBuilder('ust')
        ...
        .andWhere(`DATE_FORMAT(ust.createdAt, '%Y-%m-%d') = DATE_FORMAT(NOW(), '%Y-%m-%d')`)
        .getOne();
    } catch (error) {
   ...
   * @memberof StampRepository
   */
  async checkFirstScan(userId: string): Promise<boolean> {
    try {
      const userStamp = await this.findOne({ userId });
      if (userStamp) {
        return true;
      }
      return false;

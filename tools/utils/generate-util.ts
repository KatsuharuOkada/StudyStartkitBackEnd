...
import constantUtil from '../common/constant';
...
class GenerateUtil {
  ...
  getParamForGraphql(content, params, key, msg) {
    ...
    let typeName = '';
    let output = '';
    const type = key === constantUtil.INPUT_PARAM_KEY ? constantUtil.GRAPHQL_INPUT : constantUtil.GRAPHQL_OUTPUT;
  ...
  replaceContentForServiceAdd(content, params) {
    const lowerFolderName = _.lowerCase(params.folderName);
    const objEntity = lowerFolderName + 'ObjEntity';
    let objUpdateEntity = '';

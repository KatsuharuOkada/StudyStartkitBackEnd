  ...
  replaceContentForServiceUpdate(content, params) {
    ...
    if (params.otherTableDeleteInfo) {
      ...
      splitOtherDeleteInfo.forEach((deleteInfo) => {
        ...
        if (splitInfo.length === 2) {
          let info = 'await this.' + splitInfo[0] + '.delete(';
          info += splitInfo[1] + ');';
      ...
      splitOtherSaveInfo.forEach((saveInfo) => {
        ...
        if (splitSaveInfo.length === 5) {
          ...
          if (splitFirstInfo.length === 3) {
            const keyValue = splitFirstInfo[0],
            ...
            info += 'await this.' + splitSaveInfo[1] + ".createQueryBuilder().where('";
            if (splitFirstInfo[1] === 'array') {
              ...
              info += 'const savedObj = new ' + splitSaveInfo[3] + '();\n';
              splitLastInfo.forEach((keyInfo, idx) => {
                if (idx === 0) {
                  info += 'savedObj.' + keyInfo + ' = ' + 'response;\n';
                } else {
                  info += 'savedObj.' + keyInfo + ' = ' + 'item;\n';
                }
              });
              info += 'return this.' + splitSaveInfo[2] + '.create(savedObj);\n';
              info += '});\n';
              // Save info
              info += 'if (' + arrVariableName + '.length) {\n';
              info += 'const ' + resultVariableName + ':any = await this.' + splitSaveInfo[2] + '.save(';
